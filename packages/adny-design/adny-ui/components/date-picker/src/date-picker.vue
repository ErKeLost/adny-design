  ...
  <div class="var-date-picker" :class="[shadow ? 'var-elevation--2' : null]">
    <div class="var-date-picker-title" :style="{ background: headerColor || color }">
      <div
        ...
        @click="clickEl('year')"
      >
        <slot name="year" :year="previewYear">{{ previewYear }}</slot>
      </div>
      ...
      >
        ...
        >
          <div :key="chooseYear + chooseMonth?.index" v-if="type === 'month'">
            <slot name="range" :choose="getChoose.chooseRangeMonth" v-if="range">{{ getMonthTitle }}</slot>
            <slot
              name="multiple"
          ...
          <div :key="chooseYear + chooseMonth?.index + chooseDay" v-else>
            <slot name="range" :choose="formatRange" v-if="range">{{ getDateTitle }}</slot>
            <slot
              name="multiple"
              :choose="getChoose.chooseDays"
              v-else-if="multiple"
            >{{ getDateTitle }}</slot>
            <slot name="date" v-bind="slotProps" v-else>{{ getDateTitle }}</slot>
          </div>
        </transition>
      </div>
    </div>
...
import MonthPickerPanel from './comp/month-picker-panel.vue'
import YearPickerPanel from './comp/year-picker-panel.vue'
import DayPickerPanel from './comp/day-picker-panel.vue'
