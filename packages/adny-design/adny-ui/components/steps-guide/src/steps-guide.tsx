import "../styles/steps-guide.less";
import { computed, ref, defineComponent, Teleport, onMounted } from "vue";
import { stepsGuideProps, StepsGuideProps, Step } from "./props";
...
export default defineComponent({
  ...
  setup(props: StepsGuideProps, ctx) {
    ...
    const { stepsRef, guidePosition, guideClassList, updateGuidePosition } =
      useStepsGuidePosition(props, currentStep);
    const { stepsCount, closeGuide, setCurrentIndex } = useStepsGuideCtrl(
      props,
      ctx,
      updateGuidePosition,
      stepIndex
        ...
        <Teleport to="body">
          <div style={guidePosition} class={guideClassList} ref={stepsRef}>
            ...
            <div class="devui-guide-container">
              ...
              {props.showClose ? (
                <div class="icon icon-close" onClick={closeGuide}></div>
              ) : null}
                  ...
                  <div class="devui-dots">
                    {props.steps.map((step, index) => {
                      return (
                        <em
                          class={[
                            "icon icon-dot-status",
                            currentStep.value === step ? "devui-active" : "",
                          ]}
                          key={index}
                        ></em>
                      );
