...
import { getElement, isComponent } from "../../../utils/elements";

/**
 * 弹性的 Overlay，用于连接固定的和相对点
 */
export const FlexibleOverlay = defineComponent({
  name: "AFlexibleOverlay",
  ...
  setup(props: FlexibleOverlayProps, ctx) {
    ...
    onMounted(async () => {
      ...
      watch(
        [locationElements, visibleRef, positionRef],
        async ([locationElements, visible, position], ov, onInvalidate) => {
          ...
          const unsubscriptions = [
            ...
            subscribeOverlayResize(overlay, (entries) =>
              handleRectChange(position, entries[0].contentRect, origin)
            ),
            subscribeOriginResize(origin, () =>
              handleRectChange(
                position,
                overlay.getBoundingClientRect(),
                origin
...
function subscribeOverlayResize(
  overlay: Element,
