<template>
  <div class="adny-progress">
    <div class="adny-progress-linear" v-if="mode === 'linear'">
      <div
        class="adny-progress-linear__block"
        :style="{ height: `${lineWidth}px` }"
        v-bind="$attrs"
      >
        <div
          class="adny-progress-linear__background"
          v-if="track"
          :style="{ background: trackColor }"
        ></div>
        <div
          :class="[
            `adny-progress-linear__certain${ripple ? ' adny-progress-linear__ripple' : ''}`,
            indeterminate ? 'adny-progress-linear__certain-bar-value' : null
          ]"
          :style="{
            background: color,
            width: indeterminate ? 'indeterminateProps.width' : linearProps.width && linearProps.width
          }"
        ></div>
      </div>
      <div class="adny-progress-linear__label" v-bind="$attrs" v-if="label">
        <slot>{{ linearProps.roundValue }}</slot>
      </div>
    </div>

    <div
      class="adny-progress-circle"
      v-if="mode === 'circle'"
      :style="{ width: `${size}px`, height: `${size}px` }"
    >
      <svg
        class="adny-progress-circle__svg"
        :style="{ transform: `rotate(${rotate - 90}deg)` }"
        :viewBox="circleProps.viewBox"
      >
        <circle
          v-if="track"
          class="adny-progress-circle__background"
          :cx="size / 2"
          :cy="size / 2"
          :r="circleProps.radius"
          fill="transparent"
          :stroke-width="lineWidth"
          :style="{
            strokeDasharray: circleProps.perimeter,
            stroke: trackColor
          }"
        />
        <circle
          class="adny-progress-circle__certain"
          :cx="size / 2"
          :cy="size / 2"
          :r="circleProps.radius"
          fill="transparent"
          :stroke-width="lineWidth"
          :style="{
            strokeDasharray: circleProps.strokeDasharray,
            stroke: color
          }"
        />
      </svg>

      <div class="adny-progress-circle__label" v-if="label" v-bind="$attrs">
        <slot>{{ circleProps.roundValue }}</slot>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import { defineComponent, computed } from 'vue'
import { props } from './props'
import { toNumber } from '../../../utils/shared'

export default defineComponent({
  name: 'AProgressLinear',
  inheritAttrs: false,
  props,
  setup(props) {
    const indeterminateProps = computed(() => {
      return {
        width: '80%'
      }
    })
    const linearProps = computed(() => {
      const value = toNumber(props.value)
      const width = value > 100 ? 100 : value
      const roundValue = value > 100 ? 100 : Math.round(value)

      return {
        width: `${width}%`,
        roundValue: `${roundValue}%`
      }
    })

    const circleProps = computed(() => {
      const { size, lineWidth, value } = props
      const viewBox = `0 0 ${size} ${size}`
      const roundValue = toNumber(value) > 100 ? 100 : Math.round(toNumber(value))
      const radius = (size - toNumber(lineWidth)) / 2
      const perimeter = 2 * Math.PI * radius
      const strokeDasharray = `${(roundValue / 100) * perimeter}, ${perimeter}`

      return {
        viewBox,
        radius,
        strokeDasharray,
        perimeter,
        roundValue: `${roundValue}%`
      }
    })
    return {
      linearProps,
      circleProps,
      indeterminateProps
    }
  }
})
</script>

<style lang="less">
@import "../../../styles/common";
@import "../styles/progress";
</style>
