<template>
  <div class="ph-color-sv" :style="svStyle" ref="circle" @click="onClick">
...
export default defineComponent({
  ...
  setup(props, { emit, attrs, slots }) {
    ...
    const onClick = (e: PointerEvent) => {
      ...
      const { left, top } = circle.value.getBoundingClientRect();
      update(clientX - left, clientY - top);
    };
    onMounted(() => {
      nextTick(() => {
        if (!circle.value) return;
        state.svw = circle.value.offsetWidth;
        state.svh = circle.value.offsetHeight;
      });
    });
    return {
      onClick,
