...
export function rgb2hexstr(rgb: number[]): string {
  ...
  if (a !== undefined && a < 100) hex += n2hex((a * 255) / 100);
  return hex;
}
...
export function rgb2hsb(rgb: number[]) {
  ...
  let H = 0;
  if (max == min) H = 0;
  else if (max == r && g >= b) H = ((g - b) * 60) / (max - min);
  else if (max == r && g < b) H = ((g - b) * 60) / (max - min) + 360;
  else if (max == g) H = ((b - r) * 60) / (max - min) + 120;
  else if (max == b) H = ((r - g) * 60) / (max - min) + 240;
  H = Math.round(H || 0);
...
export function hexstr2rgb(color: string): number[] {
  ...
  res[3] =
    res[3] === undefined ? 100 : Math.round((Math.max(res[3], 0) * 100) / 255);
  return res;
}
...
export function parseColor(color: string): {
  ...
  let rgba: number[], mode: IColorMode;
